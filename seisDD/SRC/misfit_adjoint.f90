...
myrank = 0
  ...
  if (DISPLAY_DETAILS .and. myrank == 0) print *,"Running misfit_adjoint.exe on",NPROC,"processors"
  call cpu_time(t1)

  ! parse command line arguments
  if (command_argument_count() /= NARGS) then
    if (myrank == 0) then
        print *, 'USAGE:  mpirun -np NPROC bin/misfit_adjoint.exe compute_adjoint data_names input_dir'
...
! get input parameters
  ...
  if(ntype>=1) then
      ...
      if(nrec_proc > 0 ) then  ! non-zero file
          ...
          wtr=maxval(abs(seism_obs(:,:)))
          if(wtr<SMALL_VAL) cycle
          call process_obs(seism_obs)
          wtr=maxval(abs(seism_syn(:,:)))
          if(wtr<SMALL_VAL) cycle
...
subroutine process_obs(seism)
  ...
  if(wtr>SMALL_VAL) then  ! non-zero trace
      ...
      if(is_window .eq. 1 .and. istart<iend) then
          istart=max(int((dis_sr/Vmax+t0-1.2/f0)/deltat)+1,istart)
          iend=min(int((dis_sr/Vmin+3.0/f0+t0+1.2/f0)/deltat)+1,iend)
          tas(1:NSTEP)=0.d0
          if(iend>istart) then
...
use seismo_parameters
  ...
  call split_string(measurement_list,delimiter,measurement_types,ntype)
      ...
      if (norm2(d(ntstart:ntend),1)<SMALL_VAL .or. &
          ...
          (ntend-ntstart)*deltat<window_len) then 
          if(DISPLAY_DETAILS .and. compute_adjoint) then
              print*,'data trace in window is trivial on trace ',irec
              print*,'norm of data within window : ',norm2(d(ntstart:ntend),1)
              print*,'norm of syn within window : ',norm2(s(ntstart:ntend),1)
          endif
          cycle
      endif
