...
do
    ...
    sh $SCRIPTS_DIR/adjoint_source.sh $isource $NPROC_SPECFEM $compute_adjoint $data_list \
        $measurement_list $misfit_type_list $WORKING_DIR $DISK_DIR $Wscale $wavelet_path 2>./job_info/error_adj_source$isource
    ...
    if $compute_adjoint; then
        ...
        sh $SCRIPTS_DIR/Adjoint_${solver}.sh $isource $NPROC_SPECFEM $data_tag \
            $velocity_dir $SAVE_FORWARD $WORKING_DIR $DISK_DIR 2>./job_info/error_Adjoint_source$isource
    fi
