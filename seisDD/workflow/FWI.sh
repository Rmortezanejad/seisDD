...
do
    ...
    ## clean up DISK data_misfit,optimizer 
    rm -rf  $DISK_DIR/data_misfit
    rm -rf  $DISK_DIR/optimizer
...
do
    smooth_x=$(echo $(echo "$sigma_x $stage $sigma_x_rate" | awk '{ print $1-($2-1)*$3 }'))
    smooth_z=$(echo $(echo "$sigma_z $stage $sigma_z_rate" | awk '{ print $1-($2-1)*$3 }'))
    echo "          ***************************************"
    echo "          ** iteration $iter **"
...
step_length=0.0
./bin/data_misfit.exe $iter $step_length $compute_adjoint $NPROC_DATA $DISK_DIR 2> ./job_info/error_sum_misfit_$iter
...
echo "sum data misfit ...... "
mkdir -p $DISK_DIR/data_misfit
./bin/data_misfit.exe $iter $step_length $compute_adjoint $NPROC_DATA  $DISK_DIR 2> ./job_info/error_sum_misfit
...
echo " is_cont=$is_cont; is_done=$is_done; is_brak=$is_brak"
    ...
    echo "final model optimal_step_length=$optimal_step_length"
     ./bin/model_update.exe $NPROC_SPECFEM $DISK_DIR $model_list $optimal_step_length 2> ./job_info/error_update_model
      ...
      cp -r  $DISK_DIR/m_current   $SUBMIT_RESULT_stage/m_$iter
      break 
    fi
...
cp -r $DISK_DIR/data_misfit $SUBMIT_RESULT_stage/
cp -r $SUBMIT_DIR/job_info/output $SUBMIT_RESULT_stage/

echo
echo "******************finish stage $stage **************"
done # stage loop
echo
echo "******************finish all stages*******************************"
